"""
질병코딩지침서 PDF 지능형 분할 파싱
- 목차 구조를 고려하여 섹션 경계에서 분할
...
class SmartPDFSplitParser:
    ...
    def analyze_structure(self, pdf_path: Path) -> dict:
        ...
        실제 목차 추출은 복잡하므로, 여기서는 수동 정의된 섹션 경계 사용
        """
        reader = PdfReader(pdf_path)
        total_pages = len(reader.pages)

        # 수동 정의된 주요 섹션 경계 (실제 PDF 분석 결과 기반)
        # 실무에서는 첫 파싱 후 목차를 분석하여 자동 생성 가능
        major_sections = [
            {'name': '규약 및 일반지침 + I~II장', 'start': 1, 'end': 42},
            {'name': 'III~XVIII장', 'start': 43, 'end': 90},
            {'name': 'XIX~XXIII장 + 부록', 'start': 91, 'end': total_pages}
        ]
    ...
    def calculate_smart_splits(self, structure: dict) -> list:
        ...
        전략:
        1. 주요 섹션 경계를 우선 고려
        2. target_chunk_pages를 초과하지 않도록 조정
        """
        ...
        for section in major_sections:
            ...
            current_chunk_size = sum(s['end'] - s['start'] + 1 for s in current_chunk_sections)

            # 현재 섹션을 추가하면 target을 크게 초과하는 경우
            if current_chunk_sections and (current_chunk_size + section_size) > self.target_chunk_pages * 1.5:
                # 현재까지의 청크를 완성하고 새 청크 시작
                chunk_end = current_chunk_sections[-1]['end']
                splits.append({
                    'start': current_chunk_start,
                    'end': chunk_end,
                    'pages': chunk_end - current_chunk_start + 1,
                    'sections': [s['name'] for s in current_chunk_sections]
                })
    ...
    def split_pdf(self, pdf_path: Path, splits: list) -> list:
        ...
        for i, split in enumerate(splits, 1):
            ...
            print(f'           포함: {", ".join(split["sections"])}')
    ...
    def parse_chunk(self, chunk: dict, output_format: str = "html") -> dict:
        ...
        with open(chunk_path, 'rb') as f:
            ...
            try:
                ...
                print(f'       섹션: {", ".join(chunk["sections"])}')
                return result
    ...
    def merge_results(self, results: list, source_file: str, output_format: str = 'html') -> dict:
        ...
        merged = {
            ...
            'parsing_method': 'smart_section_based_split',
        ...
        for i, meta in enumerate(chunks_metadata, 1):
            ...
            print(f'     청크{i}: p.{chunk_info["start_page"]}-{chunk_info["end_page"]} '
                  f'({chunk_info["pages"]}p) - {", ".join(chunk_info["sections"])}')
    ...
    def parse_large_pdf(self, pdf_path: Path, output_format: str = 'html') -> dict:
        ...
        print(f'  주요 섹션: {len(structure["major_sections"])}개')
